...
<Aside type="note">
`assisted` is the recommended default mode during agent-driven development. Agents can verify flows even while the UI is actively changing.
</Aside>
...
<Aside type="caution">
Results in exploratory mode are clearly marked as `"mode": "exploratory"` in output. They are not CI-grade. Do not use exploratory mode as a merge gate.
</Aside>
